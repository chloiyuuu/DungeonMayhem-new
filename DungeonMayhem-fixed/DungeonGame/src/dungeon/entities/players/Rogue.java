...
public class Rogue extends Player {
    public Rogue(String name) {
        super(name, 90, 8, 22, 25, 2);
    }

    public void backstab(Damageable target) {
        System.out.println("Rogue uses Backstab from stealth!");
        target.takeDamage(getAttackDamage() + 30);
    }

    public void poisonDart(Damageable target) {
        System.out.println("Rogue shoots a Poison Dart!");
        target.takeDamage(getAttackDamage() + 10);
    }

    public void shadowClone(Damageable target) {
        System.out.println("Rogue summons a Shadow Clone to attack!");
        target.takeDamage(getAttackDamage() + 20);
    }

    @Override public void useSkillOne(Damageable target) { backstab(target); }
    @Override public void useSkillTwo(Damageable target) { poisonDart(target); }
    @Override public void useSkillThree(Damageable target) { shadowClone(target); }
}